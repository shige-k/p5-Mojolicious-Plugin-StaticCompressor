...
This Mojolicious plugin is minifier and compressor for asset file, such as the JavaScript file (.js) and CSS file (.css).
...
You can always disable a combine and minify, by using this option:

  $self->plugin('StaticCompressor', disable => 1);

(default: 0)

However, while using the disabled option, you can also use (It mean to only JUST output the source file. it's not combined and not minified.) the provided helpers (such as: 'js', 'js_nominify', and 'css', ...) from this module.

It is useful to temporary work, such as debugging.
